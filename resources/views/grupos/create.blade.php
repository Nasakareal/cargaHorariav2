...
@section('content_header')
  <h1 class="text-center w-100">Creación de un nuevo grupo</h1>
@endsection
              ...
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  <input type="text" name="group_name" id="group_name"
                         class="form-control" value="{{ old('group_name') }}" required autofocus>
                </div>
              </div>
                ...
                <div class="form-group">
                  ...
                  <select name="term_id" id="term_id" class="form-control" required>
                    ...
                    @foreach ($terminos as $t)
                      <option value="{{ $t->term_id }}" {{ old('term_id') == $t->term_id ? 'selected' : '' }}>
                        {{ $t->term_name }}
                      </option>
                    @endforeach
                  </select>
              ...
              <div class="col-md-4">
                <div class="form-group">
                  ...
                  <input type="number" name="volume" id="volume" class="form-control"
                         value="{{ old('volume', 0) }}" min="0" step="1" required>
                </div>
              </div>
                ...
                <div class="form-group">
                  ...
                  <select name="turn_id" id="turn_id" class="form-control" required>
                    ...
                    @foreach ($turnos as $sh)
                      <option value="{{ $sh->shift_id }}" {{ old('turn_id') == $sh->shift_id ? 'selected' : '' }}>
                        {{ $sh->shift_name }}
                      </option>
                    @endforeach
                  </select>
